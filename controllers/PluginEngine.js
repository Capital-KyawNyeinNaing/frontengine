...
const PluginEngine = async function () {
  fs.readFile("./cc-config.json", "utf-8", (err, data) => {
    ...
    plugin["list"].map((file) => {
      for (const list in file) {
        if (file.hasOwnProperty(list) && file["name"] !== undefined) {
          list === "js"
            ? js.push("./node_modules/" + file["name"] + "/" + file[list])
            : null;
          list === "css"
            ? css.push("./node_modules/" + file["name"] + "/" + file[list])
            : null;
        } else {
          console.error(
            "your plugin is not found, may be name is undefined\nplease check at cc-config"
          );
        }
      }
    });
    ...
    gulp
      ...
      .pipe(
        gulpif(plugin["beautify"], dest(`${res["output"]}/${plugin["dist"]}`))
      )
      .pipe(
        gulpif(
          plugin["minify"],
      ...
      .pipe(
        gulpif(plugin["minify"], dest(`${res["output"]}/${plugin["dist"]}`))
      );
    ...
    if (
      plugin["minify"] === false &&
      plugin["beautify"] === false &&
      plugin["split mode"] === false
    ) {
